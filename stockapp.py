...
def main():
    ...
    if selected == "Dashboard":
        ...
        st.info("""
        **Information**:
        - The data perform in this App is from yfinance python API, the detail you can visit(https://finance.yahoo.com/)
        """)   


        tickers_input = st.sidebar.text_area("Insert the Stock Ticker(separate with coma)", value="GOOGL, AAPL, MSFT, NVDA, TSLA, INTC")
        ...
        if tickers:
            ...
            for ticker in tickers:
                chart_types[ticker] = st.sidebar.selectbox(
                    f"Choose the plot style {ticker}",
                    ('Candlestick with Volume', 'Line Plot'),
                    index=0  
                )
            ...
            for idx, ticker in enumerate(tickers):
                ...
                with col1:
                    with st.container():
                        ...
                        if chart_types[ticker] == 'Candlestick with Volume':
        ...
        def forecast_page():
            ...
            tickers = st.sidebar.text_input("Insert the Stock Ticker(separate with coma)", "GOOGL").split(',')
